...
// File erc721a-upgradeable/contracts/ERC721A__InitializableStorage.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/ERC721A__Initializable.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/ERC721AStorage.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/IERC721AUpgradeable.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/ERC721AUpgradeable.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/extensions/IERC721AQueryableUpgradeable.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/interfaces/IERC721AQueryableUpgradeable.sol@v4.3.0
...
// File erc721a-upgradeable/contracts/interfaces/IERC721AUpgradeable.sol@v4.3.0
...
// import {IERC721GetAprBonus} from './interfaces/IERC721GetAprBonus.sol';
...
interface IERC721GetAprBonus is IERC721AQueryableUpgradeable {
    // A, S, SS, SSS respectively with 0, 1, 2, 3
    enum Tier {
        A,
        S,
        SS,
    ...
    function tokenIdToTraits(uint256) external view returns (TokenTraits memory);
}

contract NFTStakingContractFlattened is Ownable, ERC721A__IERC721ReceiverUpgradeable {
    ...
    struct UserRewards {
        ...
        uint128 rewardPerTokenCheckpoint; // RewardsPerToken the last time the user rewards were updated
    }

    uint256 public constant sharesDecimals = 1e9; // Precision factor for shares
    uint256 public unstakePeriod = 2 weeks; // Time to wait before unstaking
    IERC721GetAprBonus public immutable stakingNFT; // Contract to get the APR bonus
    IERC721AUpgradeable public immutable avatarNFT; // Contract to get the avatar staking
    uint256 public totalShares; // Total amount staked
    ...
    uint256 public rewardsPerSec; // Wei rewarded per second among all token holders
    uint256 public immutable rewardsStart; // Start of the rewards program
    uint256 public rewardsEnd; // End of the rewards program
    RewardsPerToken public rewardsPerShare; // Accumulator to track rewards per token
    mapping(address => UserRewards) public accumulatedRewards; // Rewards accumulated per user
    mapping(IERC721GetAprBonus.Tier => uint256) public tierToShares;
    mapping(address => mapping(uint256 => uint256)) public startStakingTime;
    mapping(address => uint256) public avatarStake;

    constructor(
        IERC721GetAprBonus stakingNFT_,
        IERC721AUpgradeable avatarNFT_,
        ...
        rewardsPerSec = totalRewards / (rewardsEnd_ - rewardsStart_); // The contract will fail to deploy if end <= start, as it should
        rewardsPerShare.lastUpdated = rewardsStart_.u128();
    }

    // @notice Set tierToShares
    function setTierToShares(IERC721GetAprBonus.Tier tier, uint256 shares) public onlyOwner {
        tierToShares[tier] = shares;
    }

    // @notice set unstake period
    function setUnstakePeriod(uint256 period) public onlyOwner {
        unstakePeriod = period;
    }

    // @notice set rewardsPerSec 
    function setRewardsPerSec(uint256 rewardsPerSec_) public onlyOwner {
        _updateRewardsPerShare();
        rewardsPerSec = rewardsPerSec_;
    }
    ...
    function _calculateRewardsPerShare(
        RewardsPerToken memory rewardsPerShareIn
    ) internal view returns (RewardsPerToken memory) {
        RewardsPerToken memory rewardsPerShareOut = RewardsPerToken(
            rewardsPerShareIn.accumulated,
            rewardsPerShareIn.lastUpdated
        );
        uint256 totalShares_ = totalShares;

        // No changes if the program hasn't started
        if (block.timestamp < rewardsStart) return rewardsPerShareOut;
        ...
        uint256 elapsed = updateTime - rewardsPerShareIn.lastUpdated;

        // No changes if no time has passed
        if (elapsed == 0) return rewardsPerShareOut;
        rewardsPerShareOut.lastUpdated = updateTime.u128();
        ...
        if (totalShares == 0) return rewardsPerShareOut;

        // Calculate and update the new value of the accumulator.
        rewardsPerShareOut.accumulated = (rewardsPerShareIn.accumulated +
            (sharesDecimals * elapsed * rewardsPerSec) /
            totalShares_).u128(); // The rewards per token are scaled up for precision
    ...
    function _calculateUserRewards(
        ...
        return (stake_ * (latterCheckpoint - earlierCheckpoint)) / 1e9; // We must scale down the rewards by the precision factor
    }
    ...
    function _updateRewardsPerShare() internal returns (RewardsPerToken memory) {
        RewardsPerToken memory rewardsPerShareIn = rewardsPerShare;
        RewardsPerToken memory rewardsPerShareOut = _calculateRewardsPerShare(rewardsPerShareIn);

        // We skip the storage changes if already updated in the same block, or if the program has ended and was updated at the end
        if (rewardsPerShareIn.lastUpdated == rewardsPerShareOut.lastUpdated) return rewardsPerShareOut;

        rewardsPerShare = rewardsPerShareOut;
        emit RewardsPerTokenUpdated(rewardsPerShareOut.accumulated);
    ...
    function _updateUserRewards(address user) internal returns (UserRewards memory) {
        RewardsPerToken memory rewardsPerToken_ = _updateRewardsPerShare();
        UserRewards memory userRewards_ = accumulatedRewards[user];

        // We skip the storage changes if already updated in the same block
        if (userRewards_.rewardPerTokenCheckpoint == rewardsPerToken_.lastUpdated) return userRewards_;

        // Calculate and update the new value user reserves.
        userRewards_.accumulated += _calculateUserRewards(
            userStake[user],
            userRewards_.rewardPerTokenCheckpoint,
            rewardsPerToken_.accumulated
        ).u128();
        userRewards_.rewardPerTokenCheckpoint = rewardsPerToken_.accumulated;

        accumulatedRewards[user] = userRewards_;
        emit UserRewardsUpdated(user, userRewards_.accumulated, userRewards_.rewardPerTokenCheckpoint);
    ...
    function _stake(address user, uint256 tokenId) internal {
        ...
        IERC721GetAprBonus.TokenTraits memory trait = stakingNFT.tokenIdToTraits(tokenId);
        
        uint256 shares = tierToShares[trait.t_tier];
        uint256 bonusSharesByApr = shares + (shares * trait.t_aprBonus) / 100;
        uint256 finalShares = _returnTotalBonusShares(user, bonusSharesByApr);
        totalShares += finalShares;
        userStake[user] += finalShares;
        stakingNFT.safeTransferFrom(msg.sender, address(this), tokenId);
        startStakingTime[user][tokenId] = block.timestamp;
        emit Staked(user, tokenId);
    }

    /// @notice Unstake tokens.
    function _unstake(address user, uint256 tokenId) internal {
        // TODO Send the NFT to owner checking and check the staking period,
        // if it's less than 2 weeks, penalty 10% rewards of that stake
        require(startStakingTime[user][tokenId] != 0 && block.timestamp - startStakingTime[user][tokenId] > unstakePeriod, "You can't unstake before 2 weeks");
        _updateUserRewards(user);
    ...
    // @notice stake avatar
    function _stakeAvatar(address user, uint256 tokenId) internal {
        require(avatarStake[user] == 0, 'You already staked avatar');
        _updateUserRewards(user);
        avatarNFT.safeTransferFrom(msg.sender, address(this), tokenId);
        avatarStake[user] = tokenId;
        uint256 userStake_ = userStake[user];
        uint256 bonusShares = (userStake_ * 10) / 100;
        userStake[user] += bonusShares;
        totalShares += bonusShares;
    }

    // @notice unstake avatar
    function _unstakeAvatar(address user) internal {
        uint256 tokenId = avatarStake[user];
        require(tokenId != 0, 'You already unstaked avatar');
        _updateUserRewards(user);
        avatarStake[user] = 0;
        avatarNFT.safeTransferFrom(address(this), msg.sender, tokenId);
        uint256 bonusSharesByAvatar = (userStake[user] * 10) / 110; // Cause we added 10% bonus shares
        userStake[user] -= bonusSharesByAvatar;
        totalShares -= bonusSharesByAvatar;
    }

    function _returnTotalBonusShares(address user, uint256 shares) internal view returns (uint256) {
        return avatarStake[user] != 0 ? (shares + (shares * 10) / 100) : shares;
    }
    ...
    // @notice stake avatar
    function stakeAvatar(uint256 tokenId) public virtual {
        _stakeAvatar(msg.sender, tokenId);
    }

    /// @notice unstake avatar
    function unstakeAvatar() public virtual {
        _unstakeAvatar(msg.sender);
    }

    /// @notice Calculate and return current rewards per token.
    function currentRewardsPerToken() public view returns (uint256) {
        return _calculateRewardsPerShare(rewardsPerShare).accumulated;
    }
    ...
    function currentUserRewards(address user) public view returns (uint256) {
        ...
        RewardsPerToken memory rewardsPerToken_ = _calculateRewardsPerShare(rewardsPerShare);
        return
            accumulatedRewards_.accumulated +
            _calculateUserRewards(
                userStake[user],
                accumulatedRewards_.rewardPerTokenCheckpoint,
                rewardsPerToken_.accumulated
            );
    }
